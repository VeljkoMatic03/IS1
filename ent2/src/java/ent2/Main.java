...
//    private static Queue q2;
        ...
        while (true) {
            try {
                ...
                switch(requestStr) {
                    ...
                    break;
                    case "dohvatiSnimke" :
                        reply = dohvatiAudioSnimke(em);
                        replyMsg = context.createTextMessage(reply);
                        replyMsg.setJMSCorrelationID(request.getJMSCorrelationID());
                        replyMsg.setStringProperty("header", "Naziv|Datum|Trajanje;");
                        producer.send(appToP2Topic, replyMsg);
    ...
    private static String dohvatiAudioSnimke(EntityManager em) {
        ...
        for (Audiosnimak a : audioL) {
            String audioString = "" + a.getNaziv() + "|" + a.getDatum() + "|" + a.getTrajanje();
